...
<!-- css -->
<link rel="stylesheet" href="${path}/resources/css/pet/pet.css">
<!-- pet 전용 -->
...
<!-- js -->
<script src="https://kit.fontawesome.com/11defe47b4.js"
...
<body>
							...
							<div class="join">
								<form action="insertAction.do" method="post">
									<table>
										<tr>
											<th>* 펫 이름</th>
											<td><input type="text" name="pet_name" class="input"
												placeholder="이름 작성"></td>
										</tr>
										...
										<tr>
											...
											<td colspan="2">
												<div class="radio-compact">
													<label><input type="radio" name="pet_type"
														value="강아지" onclick="get_dog()"> 강아지</label> <label><input
														type="radio" name="pet_type" value="고양이"
														onclick="get_cat()"> 고양이</label>
												</div>
											</td>
										</tr>
										...
										<tr>
											<th>상세품종</th>
											<td colspan="2" width="300px"><select id="selectBox"
												name="pet_kind" onchange="handleKindChange(this)">
													<option value=''>반려동물 종류를 선택해주세요.</option>
											</select> <!-- '기타'선택 시 --> <input type="text" id="pet_kind_custom"
												class="input" placeholder="직접입력" style="display: none;">
											</td>
										</tr>

										<tr>
											<th>* 성별</th>
											<td><select name="pet_gender" class="input">
										...
										<tr>
											...
											<td colspan="2">
												<div class="radio-compact">
													<label><input type="radio" name="pet_neutered"
														value="Y"> Y(네)</label> <label><input type="radio"
														name="pet_neutered" value="N" checked> N(아니오)</label>
												</div>
											</td>
										</tr>
										...
										<tr>
											...
											<td>
												<div style="display: flex; gap: 10px; align-items: center;">
													<input type="number" id="pet_kg" name="pet_kg"
														class="input" step="0.1" min="0" placeholder="예) 7.5"
														oninput="updateSize()"> <span id="sizeText"
														class="sizeText" style="min-width: 80px;">체급: -</span>
												</div> <input type="hidden" id="pet_size" name="pet_size">
											<!-- 서버 전송용 -->
											</td>
										</tr>
											...
											<td colspan="2" style="border-bottom: none"><br>
												<div align="right">
													...
													<input class="inputButton" type="submit" value="등록"
														id="submitBtn"> <input class="inputButton"
									...
									<div id="addedPetsWrap"
										style="margin-top: 10px; display: none;">
										<!-- 추가된 반려동물들을 보여주는 영역(처음엔 숨김) -->
										...
										<!-- 모든 추가 된 동물들을 JSON 문자열로 서버에 보내는 hidden -->
									</div>
								</form>
							</div>
							<!-- .join -->
	...
	<script>
		...
		$(function() {
			...
			// 폼 제출 시 “기타”면 값 필수 확인
			$('form[action="insertAction.do"]').on(
					'submit',
					function(e) {
						if ($('#selectBox').val() === '기타'
								&& !$('#pet_kind_custom').val().trim()) {
	...
	<script>
		...
		function renderPets() {
			...
			for (var i = 0; i < pets.length; i++) {
				var p = pets[i];
				var genderLabel = (p.pet_gender === 'M' ? '수컷'
						: (p.pet_gender === 'F' ? '암컷' : '-'));
				var neuterLabel = (p.pet_neutered === 'Y' ? 'Y(네)' : 'N(아니오)');
				var sizeLabel = p.pet_size ? esc(p.pet_size) : '-';

				var html = '<div class="pet-chip">'
						+ '<div class="chip-lines">' + '<span><b>펫 이름:</b> '
						+ esc(p.pet_name)
						+ '</span>'
						+ '<span><b>생일:</b> '
						+ esc(p.pet_birthday)
		...
		$('form[action="insertAction.do"]').on(
				...
				function() {
					...
					const hasSomething = $('input[name="pet_name"]').val()
							.trim()
							|| $('#selectBox').val()
							|| $('#pet_kind_custom').val().trim();
